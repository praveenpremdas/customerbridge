...
const ProductListTableAction: React.FC<ActionListTableProduct> = ({ id }) => {
  ...
  const handleCustomerDelete = async (id:any) => {
    try {
      const response = await crmAxiosInstance.delete(`customers/` + id);
...
export const ProductListTableDataColumn = [
  {
    name: "Profile",
    cell: (row: ProductListTableDataColumnType) => <ProductListTableProductName images={row.profilePictureUrl} name={row.fullName} />,
    sortable: true,
    grow: 2,
  },
  {
